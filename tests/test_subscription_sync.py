...
def test_the_downloaded_latch_is_cleared_with_the_subscription(sync_env):
    ...
    `steam_download_seen_at` is set only by the folder scan and has no other clearer, so
    ...
    conn = get_connection(db_path)
    conn.execute("UPDATE workshop_items SET steam_download_seen_at = 99 WHERE workshop_id IN (1, 2)")
    conn.commit()
    conn.close()
    ...
    assert _row(db_path, 1)["steam_download_seen_at"] == 99, "an id the walk saw keeps its latch"
    assert _row(db_path, 2)["steam_download_seen_at"] is None
...
def test_a_partial_read_never_clears_the_downloaded_latch(sync_env, monkeypatch):
    ...
    apply_own_subscriptions(db_path, 294100, {1, 2})
    conn = get_connection(db_path)
    conn.execute("UPDATE workshop_items SET steam_download_seen_at = 99")
    conn.commit()
    conn.close()
    configure({})
    ...
    assert _row(db_path, 2)["steam_download_seen_at"] == 99
