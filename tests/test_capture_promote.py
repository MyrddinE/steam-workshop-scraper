...
def test_promotes_a_web_capture_into_a_fixture(outbox, tmp_path):
    ...
    body_path = os.path.join(fixtures, "web", meta["body_file"])
...
def test_an_unparseable_tags_capture_promotes_into_the_api_area(outbox, tmp_path):
    ...
    assert meta["area"] == "steam_api"
    assert os.path.exists(os.path.join(fixtures, "steam_api", meta["body_file"]))
...
def test_credentials_are_scrubbed_from_the_fixture(outbox, tmp_path):
    ...
    with open(os.path.join(fixtures, "steam_api", meta["body_file"]), "rb") as handle:
        body = handle.read()
...
@pytest.mark.parametrize("field", ["body_file", "body"])
def test_a_meta_sidecar_renders_under_both_spellings(tmp_path, field):
    """Batch 7 renamed the meta ``body`` field to ``body_file``.

    ``body_file`` is the current spelling and the old ``body`` is still read, so
    a fixture corpus generated by either build renders into a test module.
    """
    fixtures = tmp_path / "fixtures"
    area = fixtures / "web"
    area.mkdir(parents=True)
    (area / "case.html").write_text("<html></html>", encoding="utf-8")
    meta = {"name": "case", "area": "web", "kind": "web_selector_miss",
            field: "case.html"}
    (area / "case.meta.json").write_text(json.dumps(meta), encoding="utf-8")

    metas = capture_promote.collect_fixtures(str(fixtures))
    source = capture_promote.render_test_module(
        metas, str(fixtures), str(tmp_path / "test_out.py"))

    assert '"body": "web/case.html"' in source
