...
Three properties carry the weight: it is **inert when the switch is off**, it is
**read-only**, and the summary is the one the startup log already writes.
"""
...
def test_route_reports_the_live_file_when_the_switch_is_on(tmp_path):
    ...
    assert "idx_wilson_favorite_score" in report["plans"]["wilson_favorite_score"]
    assert "idx_wilson_subscription_score" in report["plans"]["wilson_subscription_score"]
