...
| 10 | **Review and improve test suite coverage, structure, and quality** — 25 test files (~2991 lines) exist but `conftest.py` is minimal (2 fixtures), integration test is thin (31 lines), no full daemon pipeline E2E test, `test_tui.py` is a monolithic 592-line file, and mocking patterns may be brittle. Focus areas: add missing fixtures, parametrize repetitive tests, split oversized files, add integration paths, and ensure tests follow Arrange-Act-Assert conventions. | high | medium | high | pending |
