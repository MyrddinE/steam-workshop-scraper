"""The word list the translator draws boundary phrases from.

EFF Short Wordlist #1, created by Joseph Bonneau and published by the
Electronic Frontier Foundation under CC BY 3.0:
https://www.eff.org/files/2016/09/08/eff_short_wordlist_1.txt

Embedded as a Python tuple rather than shipped as a data file so the package needs
no packaging declaration or runtime file lookup, and copied verbatim from the
published list so it can be checked against the original. Every word is three to
...
# 1296 words, in the published order.
WORDS: tuple[str, ...] = (
