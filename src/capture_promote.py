...
def _meta_body_file(meta: dict) -> str:
    """The fixture-body filename from a ``.meta.json`` sidecar.

    Batch 7 renamed the meta field ``body`` to ``body_file`` because it holds a
    filename; a sidecar generated by an older build still uses the old key.
    """
    return meta.get("body_file") or meta.get("body") or ""
...
def render_test_module(fixtures: list, fixtures_root: str = DEFAULT_FIXTURES,
    ...
    for meta in fixtures:
        rel = os.path.join(meta["area"], _meta_body_file(meta)).replace(os.sep, "/")
        entries.append({
            "name": meta["name"],
            "area": meta["area"],
            "kind": meta.get("kind"),
