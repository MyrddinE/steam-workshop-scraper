...
| `image_answer` | STATE | The preview's **outcome**, not only its file type. A real extension (`jpg`, `png`, …) means the file exists at `images/<h1>/<h2>/<h3>/<id>.<ext>` and a URL may be built from it. A **wholly numeric** value is an HTTP status the server answered with: `404`/`410` mean the preview is permanently missing and will not be retried, any other code is recorded but still retryable. Any other token (`html`, `svg+xml`) is a content type that was not a picture this downloader can store. NULL means nothing has been recorded yet. One rule follows from this: **a URL is only ever built from a known image extension**, which `src/images.py` owns so the writer and every reader agree. |
| `is_queued_for_subscription` | QUEUE | Subscription queue flag. Set by the TUI (`s`) and by `POST /api/toggle_subscription_queue/<id>`. **The flag carries no direction**: with it set, `own_subscribed` decides — set means a removal is queued, clear means an addition. That derivation is what lets one boolean serve both, and the marker's `queued_remove` state is it (`src/subscription.py`). Cleared when the queued work is carried out or abandoned: by `mark_own_subscribed` — which the subscribe engine calls when it records a subscribe (from the POST's own `success: 1`, or from its confirmation read when `VERIFY_AFTER_SUBSCRIBE` is on) and on its already-`toggled` short-circuit — by `mark_own_unsubscribed` when a queued removal completes, by `dequeue_subscription` when the Web UI's overlay Cancel or Clear Failed drops a row without recording an outcome, and by a subscription reconcile for any item it finds already subscribed. Transient working state — it reads `0` whenever nothing is queued, which is the normal resting state, not evidence of disuse. |
| `own_subscribed` | STATE (ours) | Whether **the owner** — the account whose API key and cookies are configured — is subscribed to this item right now. Reconciled from the signed-in Workshop subscriptions page (`src/subscription_sync.py`), stamped immediately by `POST /api/subscribed/<id>`, and stamped by `src/subscribe_engine.py` when the POST's own `success: 1` is recorded, when its pre-read already shows `toggled` (no request is sent in that case), and — with `VERIFY_AFTER_SUBSCRIBE` on — when its confirmation read shows the item subscribed. Cleared by `mark_own_unsubscribed` when a queued removal is carried out (`POST /api/unsubscribed/<id>` records the same fact), and by a reconcile that finds the item gone from the list. Not to be confused with `subscriptions` / `lifetime_subscriptions`, which are item-wide counts that cannot be attributed to an account. |
...
* **`is_queued_for_subscription` is `0` in an idle database, but it is not dead.** It backs a
  working feature: the TUI (`s`) and the web UI toggle it, the Web UI's drain reads
  `GET /api/queued` and clears each entry once it has subscribed. Because it is
