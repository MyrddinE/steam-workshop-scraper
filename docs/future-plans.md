...
| View state persistence | Present | The browser keeps its own view **definition** (filter rows, sort, the `Subscribed:` overlay) in `localStorage` under the versioned `view.state.v1` entry. Nothing positional persists: no open item and no scroll offset, so a load or reset is one search at the top with the first item selected. Local state wins outright; `/api/state` is the first-visit seed only. It deliberately does not write `.tui_state.yaml`, whose shape belongs to the TUI. |
