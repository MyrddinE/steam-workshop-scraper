...
**Dynamic delay**: Same compounding pattern with `image_delay_seconds` — 100 successes shrink it by 5%, 2 consecutive failures after a streak grow it by `1.05^10`. It floors at 0.5 s and is capped at 20 s. An unrecognised MIME type and a permanent status do **not** count as failures: both answer a question about the *item* rather than about our request rate, so neither grows the delay nor breaks a run of successes. The captures proved the distinction was real — 404s outnumbered timeouts 6,517 to 163 over one window, so the delay had been moved almost entirely by the class that says nothing about being refused.
