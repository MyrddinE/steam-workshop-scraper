...
4. Merges API data with existing DB row via `_merge_and_clean_api_data`, which filters to `MERGE_ITEM_KEYS` (derived from `WORKSHOP_ITEM_COLUMNS`), remaps `creator_app_id`/`consumer_app_id` to `creator_appid`/`consumer_appid`, remaps `description` to `short_description`, and remaps the API's `time_created`/`time_updated` to `steam_created_at`/`steam_updated_at`. Unknown API keys are discarded with a log message. This is the **only** place a new item can be attributed to a creator: discovery's page response carries the `publishedfileid` alone, so the creator is first known from this item's own detail request. If the merged `creator_steamid` is a creator the owner flagged as ignored, the item is written settled (`fetch_status = -2`, all four priorities zero, its `translation_queue` rows deleted) and steps 6–8 are skipped entirely for it, as is its proposal in step 9 — no web scrape, image or translation stage is queued and its creator is not proposed for a refresh. Step 5's Wilson scores are still computed first: they are a pure computation over the returned counts, not a queue stage, and having them means un-ignoring restores a row the percentiles can colour rather than one with NULL scores nothing will refresh. The merged content and the `api_fetched_at` this merge stamped are kept, so un-ignoring the creator restores the row to `200` rather than making it fetch again. The detail request is the one cost the flag cannot save; it has already been spent by the time the creator is known.
...
9. After the batch, refreshes the batch's creator profiles in **one** `get_player_summaries` call: the distinct creators proposed by enriched items whose `creators` row is missing or older than `creator_staleness_days`. This was one request per item; the "only for enriched items" and staleness rules are unchanged. A creator the owner flagged as ignored is skipped outright — their profile stops being refreshed, so no further persona request is spent on them.
...
The merge is also where the item's creator is first known, so `_process_item` asks `_creator_is_ignored` about the merged `creator_steamid` immediately after it and, when the owner flagged that creator, calls `_settle_ignored_creator_item` — the same settled shape `ignore_item` writes — and returns before any stage is queued. `_settle_ignored_creator_item` owns that branch; the merge itself stays pure and writes nothing.
...
`api_fetched_at` is older than `creator_staleness_days` (default 90). A creator the
owner flagged as ignored is never proposed in the first place — `_refresh_creators`
skips the flag — so an ignored creator produces no persona refresh and therefore no
new `personaname_en` queue entry either.
