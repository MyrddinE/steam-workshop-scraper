...
**Dynamic delay**: `api_delay` is driven by **requests**, not items — one change per batched POST, never per item. A request that returns and parses is a success whatever its individual results say (a batch of 50 of which 10 are "not found" is a completely successful API call), and it resets the failure streak. The rule is TCP congestion control, not a safety net: every healthy request shaves `API_DECAY_STEP_SECONDS = 0.01 s` off the delay (additive increase of the request rate) and every refused request doubles it (multiplicative decrease of the rate, `API_BACKOFF_FACTOR = 2.0`), floored at `API_DELAY_FLOOR = 0.01 s` and capped at `API_DELAY_CEILING = 2 s`. Steady state is a sawtooth: the delay hovers just under the smallest value Steam does not refuse, one refusal doubles it, and the following successes walk it back down, so the client keeps probing a limit that is unknown and may move. Batch size is deliberately not folded into the delay: a refusal costs one doubling whatever ids it carried, so if Steam meters the limit per item the converged *call* delay settles at roughly N times the per-item cost and the item throughput is unchanged. The delay is a literal inter-call pause, not a target rate. The value is persisted after a back-off immediately and after a decay only once it has moved `API_DELAY_PERSIST_STEP = 0.1 s`, so the per-request step does not rewrite `config.yaml` every request.
...
**Dynamic delay**: The daemon's request-granularity rule above deliberately does not apply here. A page scrape is one request per item and cannot be batched, so the worker keeps its own per-item 100-success / 2-failure compounding rule, with its own `web_delay_seconds` config key. Only an **unknown** outcome — a transport failure, a 5xx, or a page that is neither the item's nor a recognised condition — counts as a failure and grows the delay. A found description counts as a success and resets the failure streak. A rate limit is a spent budget rather than a pacing problem and buys its own 300 s pause; a missing item and a description-less item page are answers the item gave; and a gate is a session problem a slower pace cannot fix. None of those moves the failure counter, so the scraper is not throttled for a reason a lower request rate could not address. The item page with no description must not count as a success either: it yielded nothing, so counting it as one would let the delay fall again while unknown outcomes continued. The decay stops at a **6.0 s floor** (raised from 1.0 s): the same Steam budget is shared with the owner's own hand-browsing, so when scrapes start failing the worker has to back off far enough that the Workshop is still usable manually while the daemon runs. The starting default is the floor, because a default below a floor the decay enforces would be incoherent. The floor only binds while decaying, so an installation whose persisted `web_delay_seconds` is already above 6 s keeps it; it is not reset. The 20 s ceiling and the growth rule are unchanged.
...
4. If extension can't be determined, logs a warning (including the puremagic guess), captures the served response as an image failure, and clears `needs_image = 0` without incrementing the failure counter (unknown MIME is not a transient error).
...
**Dynamic delay**: Same compounding pattern with `image_delay_seconds` — 100 successes shrink it by 5%, 2 consecutive failures after a streak grow it by `1.05^10`. It floors at 0.5 s and is capped at 20 s. Unknown MIME types do NOT count as failures (they clear the flag without delay penalties). The image delay currently sits at its 20 s ceiling not because of misconfiguration but because of the 404 loop recorded in [code-issues.md](code-issues.md); the capture above is what will let that loop be reviewed.
