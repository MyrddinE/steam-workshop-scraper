...
Every column in `VALID_SORT_COLS` is backed by a leading-column index that `_ensure_indexes` (`src/database.py`) creates from one table, `QUERY_INDEXES`; `tests/test_sort_index_invariant.py` proves the invariant on both schema paths (the fresh builder and the legacy chain). Because the expected set is data, a future sortable column added without an index fails a test rather than shipping.
