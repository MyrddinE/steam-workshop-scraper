...
| `is_queued_for_subscription` | **`0` for every row in this snapshot.** That is the resting state, not disuse. The column backs the subscription queue: the TUI and web UI set it, the Web UI's drain reads `GET /api/queued` and clears each entry once it has subscribed. Do not read this snapshot as "the feature is unused". |
