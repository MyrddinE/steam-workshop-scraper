...
* **dead_items_by_queue** — flagged in red when non-zero: the number of dead items still sitting in a queue, broken down per queue (the `translation` column counts a `translation_queue` row as well as the `translation_priority` mirror). A zero value renders as an all-clear.
