...
The route is **cached server-side** in `.cutoffs_cache.json` beside the database (atomic write, same discipline as `.daemon_state.yaml`), because the page's own cache is JavaScript module state and a reload throws it away, so a reload used to recompute — 28.3 s on the owner's live database. The key is a hash of the canonical `(filters, subscribed)` plus the database's `PRAGMA user_version`, so a changed query or a schema change misses. An entry is reused for `daemon.cutoffs_cache_seconds` (default **86400 s, 24 h**); `0` disables the cache and recomputes every time. The response body is unchanged — the cache state rides in headers instead: `X-Cutoffs-Cache: hit|miss` and `X-Cutoffs-Cache-Age: <seconds>` (the entry's age, `0` on a miss). A missing, unreadable or corrupt file is never an error: it recomputes and rewrites. The client's in-memory cache stays as it is; it still saves the round trip within a session.
