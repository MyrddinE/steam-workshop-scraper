...
Read-only report on the live database's search-sort path, for answering a "sort X is slow" report with a measurement instead of a hypothesis (it opened on the owner's "subscriber score is not indexed"). The route is **inert unless `daemon.capture_web_ui_trace` is on** — it rides the UI-trace switch rather than adding a third capture key — and with the switch off it answers 404 before opening the database. With it on, `src/sort_diagnostic.py` opens the file `mode=ro` (no write, no journal-mode switch, no file creation, safe beside the running daemon) and returns `{rows, sorts, page, indexes, score_coverage, sqlite_stat1, plans, timings}`. `indexes` is `{expected, missing, wrong_definition, details}`: `details[<name>]` carries the index's live `columns` from `PRAGMA index_info`, its `unique` and `partial` flags from `PRAGMA index_list`, the `expected_columns`, and a `status` of `present`, `missing` or `wrong_definition` — because a same-named index on the wrong column is a case the planner cannot use and `CREATE INDEX IF NOT EXISTS` would never repair, so reporting the name alone was not enough. `plans[<column>]` is `{expected_index, uses_index, plan}` for each `VALID_SORT_COLS` column, so "does the plan use the index" is a boolean rather than something read out of the plan text by eye (`uses_index` is `null` for `workshop_id`, the rowid alias with no named index). The rest is unchanged: the non-NULL fraction of each score column, whether `sqlite_stat1` exists and its rows for those indexes, and the wall-clock first-page and deep-page (offset 50,000) time per sort column. `init_webserver` logs the same summary once at startup, naming a missing index, a wrong definition or a temp B-tree plan at WARNING. It writes no tree of its own; the measurements behind it are in [search-filter.md](search-filter.md#sort-indexes-the-subscriber-score-is-slow-investigation).

### `/api/cutoffs` — POST

Wilson score percentile thresholds. Accepts `{filters, subscribed}` (filters excluding percentile filters). The overlay is included so the percentiles describe the same population the grid shows. Returns `{wilson_favorite_p99, wilson_favorite_p90, ...}` — exactly those ten values, unchanged. Computed in one pass from exact percentile ranks (`percentile_disc`), not two `NTILE(100)` windows — **measured 2026-09-22 as 2.25 s against 13.24 s** on the 2.5 M-row copy, with identical values ([search-filter.md](search-filter.md#compute_wilson_cutoffs-cost-database)).

The route is **cached server-side** in `.cutoffs_cache.json` beside the database (atomic write, same discipline as `.daemon_state.yaml`), because the page's own cache is JavaScript module state and a reload throws it away, so a reload used to recompute — 28.3 s on the owner's live database. The key is a hash of the canonical `(filters, subscribed)` plus the database's `PRAGMA user_version`, so a changed query or a schema change misses and computes synchronously; a missing, unreadable or corrupt file is never an error, it just recomputes and rewrites. `daemon.cutoffs_cache_seconds` (default **86400 s, 24 h**) is a **freshness threshold, not a lifetime**: an entry older than it is **stale-while-revalidate** — the pull that finds it is answered from the stale values immediately, so the grid keeps its highlighting instead of waiting, and that same pull starts a single background regeneration so the next pull finds fresh values. At most one regeneration per key runs **per server process** — the in-flight registry is in memory, so a burst of stale pulls shares one query. Two web-server processes (the TUI's own server plus a standalone `src/web_runner.py`) could therefore run one each; that costs a duplicated query and never a wrong value, because the registry marks nothing on disk and `store` publishes atomically only on success. Nothing prunes the file by age: the entry is replaced only when a regeneration succeeds, a failed regeneration leaves it in place for the next pull to retry, and a process that dies mid-regeneration leaves it intact — highlighting degrades to stale, never to nothing. The first-time compute is the one that blocks, and on a SQLite without `percentile_disc` that is the slower `NTILE` path, which is exactly where serving stale matters most. `0` disables the cache and recomputes every time. The response body is unchanged — the cache state rides in headers instead: `X-Cutoffs-Cache: hit|stale|miss` and `X-Cutoffs-Cache-Age: <seconds>` (the entry's age, `0` on a miss). The client's in-memory cache stays as it is; it still saves the round trip within a session.
