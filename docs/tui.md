...
**Rotation is manual only**: nothing here runs on a timer, on a size threshold, or at startup, because the owner keeps a persistent `tail` in another window. Pressing the button renames the live log to `logs/<stem>-<UTC stamp>.log.gz` beside it, immediately leaves a fresh empty file at the original path, and compresses the archive on a background worker, so the event loop keeps running. While that compression runs — tens of seconds for a production-sized log — the readout reads `Rotating… (<size>)`; when it finishes the outcome line reads `Rotated: logs/<name> (<size>)`, or names the uncompressed file if compression failed. The full mechanism, including the generation marker that makes the daemon's own handler and the TUI's reopen the fresh file, is in [config-security.md](config-security.md#manual-rotation-and-why-it-is-manual). **A `tail -f` on the file follows the old descriptor and will appear to stop; a follow-by-name tail picks the new file up** — the operator accepts that, which is why the rotation is theirs to start.
