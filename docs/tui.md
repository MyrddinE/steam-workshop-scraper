...
| `item_counts` | live/dead/ignored item counts with the overall total; `alive` is the live population, so the two settled statuses are excluded rather than only death being subtracted |
...
Owns the daemon process for both UIs: `start`, `stop`, `restart`, `status`, `read_pid`, `is_running` and `tail_log`. It launches `python -m src.daemon_runner <config> --daemon` detached (`DETACHED_PROCESS` on Windows, `DEVNULL` stdio elsewhere) and keeps the Popen handle (`DaemonController.proc`) for liveness and forced shutdown. A spawned daemon that the runner's PID-file guard refuses — the file is created exclusively, and its existence blocks a start — exits non-zero before it detaches, so `start()` waits for that exit and returns the refusal rather than the success it used to claim, naming the PID file and the PID inside it; on Windows, where there is no fork, a live PID published in the file is the success signal instead.

The stop half of the PID-file protocol is unchanged: stop deletes `.daemon.pid`, which is the channel that asks a daemon to shut down regardless of who launched it. A process this controller started itself is also signalled through its Popen handle; a PID merely read out of the file is never signalled, because a stale or corrupted file can name any process at all. It waits up to `STOP_TIMEOUT_SECONDS` (220 s, derived from the daemon's worst case — a 15 s main-thread request or busy-timeout wait, the daemon's 20 s join budget, a measured 180 s allowance for the closing database snapshot and a 5 s margin) for exit, then escalates — terminate/kill on the Popen handle it owns, or, when it owns no process, an honest "did not exit" instead of killing the PID from the file. The daemon's side of that grace is bounded rather than serialised: it signals every worker together and joins them under one shared `SHUTDOWN_BUDGET_SECONDS` (20 s) deadline, naming any thread still alive instead of giving each one its own timeout in turn; the closing snapshot that runs after those joins is the one step outside that budget, which is why it has its own allowance. See [threading.md](threading.md). `start` while running and `stop` while stopped are idempotent no-ops; a start the new guard refuses is not.
...
Every transition runs on a worker thread (`_begin_transition`), because each one blocks: `stop()` polls the process every half second for up to `STOP_TIMEOUT_SECONDS` (220 s) and then waits up to another 3 s for a forced kill, and `restart()` is `stop()` followed by `start()`. Called straight from the button handler that held the Textual event loop for the whole shutdown — no keypress, no screen change and no timer, including the log poll below, which fell silent at the moment its output was most wanted. The result comes back through `call_from_thread`, the three transition buttons are disabled while one is in flight so a second press cannot start an overlapping shutdown, and `restart` is passed to the worker as a single call rather than stop-then-start so the halves cannot interleave. A controller fault puts the controls back rather than leaving the screen disabled with no way out. A start the daemon refused comes back through the same path as a warning notification carrying the controller's sentence — which names the PID file and the PID inside it — so the screen does not report a start that did not happen.
...
Above the pane, the same two-second tick also draws the **log size readout** and the **Rotate Log** button, and the line under the button holds the last rotation's outcome. The readout line and the button's label come from `src/log_rotation.py` — the readout through `DaemonController.log_status`, the label through `ROTATE_BUTTON_LABEL` — the same values the web panel's `/api/daemon` poll renders, so the two front ends show the same number and the same words. The button is disabled while a rotation is running, in either process, and when no log file is configured.

**Rotation is manual only**: nothing here runs on a timer, on a size threshold, or at startup, because the owner keeps a persistent `tail` in another window. Pressing the button renames the live log to `logs/<stem>-<UTC stamp>.log.gz` beside it, immediately leaves a fresh empty file at the original path, and compresses the archive on a background worker, so the event loop keeps running. While that compression runs — tens of seconds for a production-sized log — the readout reads `Rotating… (<size>)`; when it finishes the outcome line reads `Rotated: logs/<name> (<size>)`, or names the uncompressed file if compression failed. The full mechanism, including the generation marker that makes the daemon's own handler and the TUI's reopen the fresh file, is in [config-security.md](config-security.md#manual-rotation-and-why-it-is-manual). Rotation awareness never blocks logging: a marker read that fails for any reason warns and degrades to no rotation awareness, and a rotation-aware handler that cannot be built falls back to a plain `logging.FileHandler`, so the TUI still logs. **A `tail -f` on the file follows the old descriptor and will appear to stop; a follow-by-name tail picks the new file up** — the operator accepts that, which is why the rotation is theirs to start.
