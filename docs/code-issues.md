...
* **Checked against source**: `f898317`.
...
| The description scraper sent no cookies | It sends `sessionid` and `steamLoginSecure`; a refreshed cookie reaches the daemon via `/api/sessionid` persisting to config — [config-security.md](config-security.md) |
