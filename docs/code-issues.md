...
**The TUI prints its own markup in the translation-requested notice** — *Open*, Low

The detail pane prepends the notice to the item's description as a markdown blockquote that carries Rich markup — `f"> *[yellow]Translation requested, currently in queue...[/yellow]*"` (`src/tui.py:1653`) — and the widget rendering it is Textual's `Markdown` (`#detail-content`, `:1467`), which does not interpret Rich tags. The blockquote and the italics render; `[yellow]` and `[/yellow]` are shown to the reader as text. The web UI surfaces the same fact as its own styled element (`templates/index.html:1190`), so this is a parity gap as well as a rendering defect: both front ends know the item is queued for translation, and only one of them says so cleanly. [tui.md](tui.md), [web-ui.md](web-ui.md)

### Issue 44

**A lock that outlives the busy timeout would end a scraper thread** — *Open*, Low

Removing the per-connection journal-mode pragma (issue 43) removed the common way a lock reached an unattended reader, but it gave the daemon's worker loops no guard of their own. `WebScraperThread` and `ImageDownloadThread` carry no loop-level `try`, and `_process_item`'s writes on the main loop are equally unguarded, so a lock that outlives the connection's 15 s busy timeout raises inside a thread — which ends that worker for the life of the process, with the crash hook recording the traceback and nothing restarting it. `DiscoveryThread` and `TranslatorThread` already catch per pass, so the failure handling is not uniform, which is worth naming rather than assuming. Nothing here is known to have happened: it needs write contention that survives 15 s, where the removed pragma needed only an ordinary moment of exclusion. [threading.md](threading.md) describes each thread's own handling.

### Issue 48

**A TUI test is defined inside a skipped test and never runs** — *Open*, Low
...
**A field the model does not translate keeps its place at the head** — *Open*, Low

`_translate_batch` counts a reply that misses some rows as a success — the missed rows keep their `translation_queue` entries “and a later pass picks them up” — and logs one `No translation returned for <entity>_<field>` warning. Nothing changes the row's `priority`, so a field the model omitted keeps its place at the head and is re-sent ahead of work that has not been attempted yet; whether it eventually blocks or starves depends only on what the producers add meanwhile. *Measured live*: one miss in the last 60,000 log lines, for a single field, with the queue since drained past it — so it is holding nothing up, but it is not being demoted either, and nothing records that it is still being retried. **Owner's decision, 2026-09-20:** lower the missed row's priority by one step and leave it queued — never remove it and never give up on it. At temperature 0 the input is still not identical between attempts, because the batch's other fields change and the activation levels with them, so one miss is not evidence of permanent failure. A **whole-batch** failure is the other case and stays as it is: those rows are left untouched, because that failure is unlikely to be content-related. The remaining diagnostic gap is that such a failure names the exception but not the rows it sent, although `_row_key` exists. [data-pipeline.md](data-pipeline.md), [data-model.md](data-model.md)
