...
| 4 | Image work is re-queued on every fetch, changed or not | Open | Medium | `_flag_scrape_and_image` flags an image download whenever `preview_url` is present (`src/daemon.py:468`), with no comparison against the previous revision, so every successful API fetch re-queues the preview — including the periodic staleness refresh, which by design re-fetches unchanged items. The HTML scrape beside it is already gated: it compares the pre-fetch `steam_updated_at` against the freshly merged one and reuses the stored description when they match (`src/daemon.py:455`). The API refresh is meant to be the change detector that the dependent stages follow. [timestamps.md](timestamps.md), [data-pipeline.md](data-pipeline.md). |
...
| Priority decay was asymmetric across queues | Superseded: the intended model is change-driven, not per-queue sweeps; the surviving gap is image (#4) |
