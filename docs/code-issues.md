...
| 2 | The test suite only passes with a stray `test.db` present | Open | Medium | `tests/test_daemon.py` points 22 tests at a relative `test.db` (`tests/test_daemon.py:11`) and constructs `Daemon`, whose `__init__` reads the `app_tracking` table (`src/daemon.py:245`). Nothing initialises that file, so on a clean checkout it is created empty on first connect and **15 tests fail** with `no such table: app_tracking`. They pass only because a schema-bearing `test.db` is left in the working directory by earlier runs, which is how the suite came to be reported green: `461 passed` in the working tree is `446 passed, 15 failed` from a fresh clone. A new worktree reproduces it every time. There is no document covering the test suite, so this entry carries its own context: the suite is not hermetic and its baseline is not reproducible from a clean checkout. |
