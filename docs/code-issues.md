...
**A lock that outlives the busy timeout kills the daemon** — *Open*, High

Removing the per-connection journal-mode pragma (issue 43) removed the common way a lock reached an unattended reader, but it left the daemon's loops unevenly guarded. `DiscoveryThread` and `TranslatorThread` catch per pass; `WebScraperThread` has no loop-level `try` at all, `ImageDownloadThread` wraps only its request — `get_next_image_item` and the no-URL write sit outside it — and the main loop calls `process_batch()` bare (`src/daemon.py:1322`). A lock outliving the connection's 15 s busy timeout therefore escapes to the top instead of costing one iteration. **Observed live, 2026-09-20 08:52:** `raise_web_scrape_priority` (`src/database.py:3928`, at its `conn.execute` on `:3931`) raised `sqlite3.OperationalError: database is locked` under `_raise_scrape_and_image_priorities` (`src/daemon.py:1131`); it propagated through `_process_item` and `process_batch` out of `run()`, the crash hook logged `Unhandled OperationalError in the daemon process` (`src/crash.py:330`), and the daemon stopped — it is detached, and nothing restarts it. **Four occurrences in the 34 hours from 2026-09-19 04:06 to 2026-09-20 13:52 UTC** now stand in the outbox crash dumps. The log's local clock runs five hours behind UTC, so that 08:52 log line *is* the 13:52 dump — one event, not two. Two escapes were from the main loop and killed the daemon: 2026-09-20 13:52 at `raise_web_scrape_priority`, and 2026-09-19 13:58 at the older `flag_for_web_scrape` site. The other two — 2026-09-19 04:06 and 18:00 — escaped `get_next_image_item` on the image worker thread, where `threading.excepthook` records a dump and returns: **the daemon survives and the image worker is silently gone**, because `run()` never checks worker liveness, so image downloads stop until someone restarts it. That quieter half is the same missing guard, and the title covers only the louder one. What held the write lock past 15 s is not established by any of the dumps and is a separate question from the missing guard this entry records. [threading.md](threading.md) describes each thread's own handling.
