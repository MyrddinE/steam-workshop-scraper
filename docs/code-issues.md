...
**A snapshot can start with no room to finish** — *Open*, Low

The stale artefact this entry was opened for is **gone**: `<outbox>/db/workshop-backup.db.gz` — 672.9 MB, written 2026-09-12 — was removed by hand, and *measured live* on 2026-09-21 the directory holds only `workshop-backup.db` (3.44 GB). Nothing in the build ever wrote, read or deleted that file (`DB_SNAPSHOT_REL_PATH` is `db/workshop-backup.db`, `src/backup.py:28`, and no `gzip` path remains), and `_warn_about_stale_artefacts` (`src/backup.py:185`) would name any file in that directory the build does not manage, once per process: it reports rather than deletes, because a backup artefact is exactly the kind of file that may have been kept on purpose. That guard stays useful now the file is gone — it is what would notice the next leftover.
