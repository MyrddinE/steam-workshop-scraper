...
| `daemon.api_batch_size` | int | 10 | Items to process per `process_batch` iteration. `daemon.batch_size` is an accepted legacy alias, honoured with a deprecation warning. |
...
| `daemon.creator_staleness_days` | int | 90 | Days before a creator profile is re-fetched from Steam. `daemon.user_staleness_days` is an accepted legacy alias, honoured with a deprecation warning. |
