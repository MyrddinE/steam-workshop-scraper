...
| `daemon.cutoffs_cache_seconds` | number | 86400 | How long `POST /api/cutoffs` may reuse its persisted percentile-cutoff entry, kept in `.cutoffs_cache.json` beside the database (atomic write, the same state-file discipline as `.daemon_state.yaml`). **Read by the web server**, from the `daemon` section like the capture switches; the daemon itself does not use it. `0` disables caching, so every request recomputes. The entry is keyed by the query and the database's `user_version`, so a changed query or schema change misses regardless. See [web-ui.md](web-ui.md#api-cutoffs--post). |
